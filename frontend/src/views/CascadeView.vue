...
const run = async () => {
  ...
  result.value = unwrap(await api.post('/detect/cascade', { case_id: 'case_threat_intel_false_consensus' }))
}
</script>
<template>
  <div class="page-head"><h1>级联错误检测</h1><p>检测错误 Claim 的传播、伪多数共识、语义漂移、影响力与 Byzantine 可疑度。</p></div>
  <section class="panel"><el-button type="primary" @click="run">运行级联检测</el-button></section>
  <template v-if="result">
    ...
    <section class="panel"><GraphChart :graph="result.graph" /></section>
