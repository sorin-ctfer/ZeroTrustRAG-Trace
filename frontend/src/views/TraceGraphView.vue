...
const run = async () => {
  ...
  result.value = unwrap(await api.post('/trace/ipjg', { case_id: 'case_threat_intel_false_consensus' }))
}
</script>
<template>
  <div class="page-head"><h1>信息污染联合溯源</h1><p>IPJG 四层图谱从错误 Action 反向追踪 Consensus、Claim、Evidence 与 Chunk。</p></div>
  <section class="panel"><el-button type="primary" @click="run">构建 IPJG 图谱</el-button></section>
  <section v-if="result" class="panel"><GraphChart :graph="result" /></section>
